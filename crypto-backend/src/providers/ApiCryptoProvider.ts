...
export class ApiCryptoProvider extends CryptoProvider {
  ...
  async addCrypto(
    ...
    try {
      ...
      logger.error(
        `Error al agregar la criptomoneda para el usuario ${userId}: ${error}`
      );
  ...
  async getCryptos(): Promise<Crypto[]> {
    try {
      ...
      logger.error(
        `Error al obtener criptomonedas de la API externa: ${error}`
      );
  ...
  async getUserCryptos(userId: string): Promise<Crypto[]> {
    try {
      ...
      logger.error(
        `Error al obtener criptomonedas del usuario ${userId}: ${error}`
      );
  ...
  async deleteCrypto(cryptoId: string, userId: string): Promise<void> {
    try {
      ...
      logger.info(
        `Criptomoneda eliminada para el usuario ${userId}: ID ${cryptoId}`
      );
    } catch (error) {
      logger.error(
        `Error al eliminar la criptomoneda para el usuario ${userId}: ${error}`
      );
  ...
  async getCryptoDetails(
    ...
    try {
      ...
      if (!crypto) {
        logger.warn(
          `Criptomoneda no encontrada para el usuario ${userId}: ID ${id}`
        );
        throw new Error("Criptomoneda no encontrada");
      }

      const historyResponse = await axios.get(
        `https://api.coincap.io/v2/assets/${id}/history?interval=d1&start=${Date.now() - 6 * 30 * 24 * 60 * 60 * 1000}&end=${Date.now()}`
      );
      logger.info(
        `Detalles de la criptomoneda obtenidos para el usuario ${userId}: ${crypto.name}`
      );
      ...
      logger.error(
        `Error al obtener detalles de la criptomoneda para el usuario ${userId}: ${error}`
      );
    ...
    try {
      ...
      for (const alert of activeAlerts) {
        const currentPrice = await this.getCurrentPrice(
          alert.cryptocurrency.id
        );
        const priceChange =
          ((currentPrice - alert.cryptocurrency.price) /
  ...
  async updateAlert(
    alertId: string,
    userId: string,
    updates: { isActive?: boolean; thresholdPercentage?: number }
  ): Promise<CryptoAlert> {
    try {
